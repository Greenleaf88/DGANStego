...
class DatasetFromFolder2(data.Dataset):
    def __init__(self, image_dir, subfolder='train.txt', transform=None, resize_scale=None, crop_size=None,
        ...
        self.secret_images = image_names[:len(image_names)//2]
        self.cover_images = image_names[len(image_names)//2:]
        print(len(self.secret_images))
        print(len(self.cover_images))
        # np.save(np.array(self.secret_images), 'secret_images.npy')
        # np.save(np.array(self.cover_images), 'cover_images.npy')
        self.transform = transform
        self.resize_scale = resize_scale
        self.crop_size = crop_size
        self.fliplr = fliplr
    ...
    def __getitem__(self, index):
        ...
        cover_fn = os.path.join(self.input_path, self.cover_images[index])
        secret = Image.open(secret_fn).convert('L')
        # secret = Image.open(secret_fn).convert('RGB')
        cover = Image.open(cover_fn).convert('RGB')
        # target = Image.open(cover_fn).convert('RGB')


        if self.crop_size:
            if cover.size[0] >= self.crop_size:
                x = random.randint(0,  self.crop_size - cover.size[0] + 1)
                cover = cover.crop(x, 0, x + self.crop_size, cover.size[1])
            if cover.size[1] >= self.crop_size:
                y = random.randint(0, self.crop_size - cover.size[1] + 1)
                cover = cover.crop(0, y , cover.size[0], y + self.crop_size)
        ...
        # preprocessing\
            ...
            if secret.size[0] >= self.crop_size:
                x = random.randint(0, self.crop_size - cover.size[0] + 1)
                secret = secret.crop(x, 0, x + self.crop_size, cover.size[1])
            if secret.size[1] >= self.crop_size:
                y = random.randint(0, self.crop_size - cover.size[1] + 1)
                secret = secret.crop(0, y, cover.size[0], y + self.crop_size)
            else:
                if self.resize_scale:
                    secret = secret.resize((self.resize_scale, self.resize_scale), Image.BILINEAR)
                    # cover = cover.resize((self.resize_scale, self.resize_scale), Image.BILINEAR)
