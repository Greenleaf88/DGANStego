...
def saveStegos(stegos, fns, save_dir='./'):
    ...
    stegos = [to_np(i) for i in stegos]
    img = np.squeeze(stegos[0])
    img = (((img - img.min()) * 255) / (img.max() - img.min())).transpose(1, 2, 0).astype(np.uint8)
    img = Image.fromarray(img, 'RGB')
    img.save(os.path.join(save_dir, fns[0]))

    img = np.squeeze(stegos[1])
    img = (((img - img.min()) * 255) / (img.max() - img.min())).astype(np.uint8)
    img = Image.fromarray(img)
    img.save(os.path.join(save_dir, 'secret_'+fns[0]))
...
def plotCoverSecret(covers, stegos, secrets, secretsRevealed, epoch, save=True, save_dir='results/',
    ...
    # np.save('secrets.npy', secrets)
    # np.save('secrets_revealed.npy', secretsRevealed)
...
def plot_validation_scores(log_path):
    ...
    scores = [i.strip().split(' ') for i in log if i.startswith('This epoch')]

    en_sim = [float(i[3]) for i in scores]
    de_sim = [float(i[5]) for i in scores]
    en_psnr = [float(i[-3]) for i in scores]
    de_psnr = [float(i[-1]) for i in scores]
    ...
    plt.legend(handles=[l1, l3], labels=['cover', 'secret'],
               loc='best')
    plt.savefig('{:s}_result.jpg'.format('ssim'))

    plt.figure()
    n = len(en_sim)
    x = np.linspace(1, n, n)
    l2, = plt.plot(x, en_psnr, label='parabola', color='red', linewidth=1.0, linestyle='--')
    l4, = plt.plot(x, de_psnr, label='parabola', color='blue', linewidth=1.0, linestyle='--')

    plt.xlabel('epoch')
    plt.ylabel('psnr')

    plt.legend(handles=[l2, l4], labels=['cover', 'secret'],
               loc='best')
    plt.savefig('{:s}_result.jpg'.format('psnr'))

if __name__ == '__main__':

    plot_validation_scores('VOC2012_results/log')
